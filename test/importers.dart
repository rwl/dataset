...
verifyImport(obj, strictData) {
  ...
  ok(/*typeof*/ strictData._rowPositionById != "undefined",
      "row position by id cache exists");
  ok(/*typeof*/ strictData._rowIdByPosition != "undefined",
      "row id by position cache exists");
  ok(/*typeof*/ strictData._columnPositionByName != "undefined",
      "column position by name cache exists");
  ...
  for (var i = 0; i < strictData.length; i++) {
    ...
    ok(i == strictData._rowPositionById[id],
        "row " + i + " id correctly cached");
  }

  // verify all rows have the proper amount of data
  _.each(strictData._columns, (column) {
    ok(column.data.length == strictData.length,
        "row count for column " + column.name + " is correct.");
  });

  // Verify all column position have been set.
  _.each(strictData._columns, (column, i) {
    ok(strictData._columnPositionByName[column.name] == i,
        "proper column position has been set");
  });
...
checkColumnTypes(strictData) {
  ...
  ok(strictData._column('character').type == "string",
      "character is string type");
  ok(strictData._column('name').type == "string", "name is string type");
  ok(strictData._column('is_modern').type == "boolean",
      "is_modern is boolean type");
  ok(strictData._column('numeric_value').type == "number",
      "numeric_value is numeric type");
}
...
importersTest() {
  test("Basic Strict Import through Dataset API", 47, () {
    var ds = new Dataset({data: Miso.alphabet_strict, strict: true});
    _.when(ds.fetch()).then(() {
      verifyImport(Miso.alphabet_strict, ds);
      equals(/*typeof*/ ds.columns, "function",
          "columns is the function, not the columns obj");
    });
  });

  test("Basic Strict Import through Dataset API with custom idAttribute", 44,
      () {
    var ds = new Miso.Dataset(
        {data: Miso.alphabet_strict, strict: true, idAttribute: "character"});
    _.when(ds.fetch()).then(() {
      verifyImport(Miso.alphabet_strict, ds);
      equals(/*typeof*/ ds.columns, "function",
          "columns is the function, not the columns obj");
  ...
  test("Manual column type override", () {
    ...
    _.when(ds.fetch()).then(() {
      ok(ds._column('numeric_value').type == "string",
          "numeric_value is type string");
      ok(_.uniq(ds._column('numeric_value').data)[0] == "1",
          "numeric_value has been coerced to string");
    });
  });
  ...
  test("Manual column type override", () {
    ...
    data.columns[1].data = [];
    _(data.columns[0].data.length).times(() {
    ...
    _.when(ds.fetch()).then(() {
      ...
      ok(ds._column('name').data[0].version == moment().version,
          "is a moment object");
    });
  });
  ...
  test("Manual column type override with extra properties", () {
    ...
    _.when(ds.fetch()).then(() {
      ok(ds._column('character').type == "time",
          "character column has a type of time");
      // verify format was properly coerced
      equals(ds._column('character').data[0].valueOf(),
          moment("12/31 2012", "MM/DD YYYY"));
    });
  });
  ...
  test("Basic json url fetch through Dataset API with custom idAttribute", 43,
      () {
    var url = "data/alphabet_strict.json";
    var ds = new Miso.Dataset({
      url: url,
  ...
  test("Basic jsonp url fetch with Dataset API without setting callback=", 46,
      () {
  ...
  test("Basic delimiter parsing test with Dataset API via url", 46, () {
    ...
    ds.fetch({
      success: () {
        verifyImport(Miso.alphabet_strict, self);
        start();
      }
    });
  });
  ...
  test(
      "Basic delimiter parsing test with custom separator with Dataset API", 46,
      () {
    var ds = new Dataset(
  ...
  function verifyGoogleSpreadsheet(d, obj) {
    ok(_.isEqual(
        _.keys(d._columnPositionByName), _.keys(obj._columnPositionByName)));
    ok(_.isEqual(d._rowIdByPosition.length, obj._rowIdByPosition.length));

    // ignoring id column, since that changes.
    for (var i = 1; i < d.length; i++) {
      ok(
          _.isEqual(d._columns[i].data, obj._columns[i].data),
          "Expected: " +
              d._columns[i].data +
              " Got: " +
              Miso.google_spreadsheet_strict._columns[i].data);
    }
  }
  ...
  test("Google spreadsheet dataset test", () {
    ...
    var ds = new Dataset({
      ...
      worksheet: worksheet,
      ready: () {
        verifyGoogleSpreadsheet(self, Miso.google_spreadsheet_strict);
        start();
      }
    });
    ds.fetch();
  ...
  test("Google spreadsheet fast parsing", () {
    ...
    ds.fetch({
      success: () {
        equals(ds.length, 6);
        ok(self._columns.length == 3);
        ok(_.isEqual(
            ds.column("State").data, ["AZ", "AZ", "AZ", "MA", "MA", "MA"]));
  ...
  test("Basic polling importer api", () {
    ...
    importer.fetch({
      success: (data) {
        ...
        if (madereqs == reqs) {
          importer.stop();
          ok(data[0].id == (initId + (madereqs * 10)),
              data[0].id + "," + (initId + (madereqs * 10)));
          start();
        } else if (madereqs == 0) {
          initId = data[0].id;
        } else {
    ...
    ds.fetch({
      success: () {
        ...
        if (madereqs == reqs) {
          ...
          equals(ds._columns[1].data[expectedSize - 1],
              (startId + expectedSize - 1) + "_key");
          equals(ds._columns[2].data[expectedSize - 1],
              (startId + expectedSize - 1) + "_value");
          equals(ds._columns[3].data[expectedSize - 1],
              (startId + expectedSize - 1));
          ...
          ok(_.isEqual(
              ds._columnPositionByName, {_id: 0, id: 3, key: 1, value: 2}));
  ...
  test("Polling with unique constraint for updates", 32, () {
    ...
    function verifyEvents() {
      ...
      equals(events.length / 3, requests - 1,
          'one less set of update events than reqs');
      _(requests).times((i) {
    ...
    ds.fetch({
      success: () {
        ...
        if (madereqs == reqs) {
          ...
          ok(_.isEqual(idscol, self.column("id").data));
          ok(_.isEqual(keycol, self.column("key").data));
          ok(_.isEqual(valcol, self.column("value").data));
