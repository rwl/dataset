...
class GoogleSpreadsheet implements Parser {
  /**
  ...
  parse(data) {
    var columns = [], columnData = [], keyedData = {}, i;

    // the fast importer API is not available
    if (/*typeof*/ data.status != "undefined" && data.status == "error") {
      throw new Error(
          "You can't use the fast importer for this url. Disable the fast flag");
    }
    ...
    if (this.fast) {
      ...
      _.each(columns, (colName, index) {
        keyedData[colName] = columnData[index];
      });
    } else {
      var positionRegex = r"/([A-Z]+)(\d+)", columnPositions = {};

      _.each(data.feed.entry, (cell) {
        ...
        if (position == 1) {
          ...
          if (columns.indexOf(cell.content.$t) != -1) {
            throw new Error("You have more than one column named \"" +
                cell.content.$t +
                "\"");
          } else {
      ...
      _.each(columnData, (coldata, column) {
    ...
    return {columns: columns, data: keyedData};
  }
}
